locals {
  template_name = "opnsense-${var.environment}-${var.node_suffix}-template"
  vm_name = "opnsense-${var.environment}-${var.node_suffix}"
  vm_id = var.vm_id
}
...
source "proxmox-iso" "opnsense" {
  ...
  vm_id    = local.vm_id
  template_description = "OPNsense Template - ${var.environment}/${var.node_suffix}"
